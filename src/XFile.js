...
export default class XFiler {
  ...
  async deletePic (socket, req) {
    ...
    let picpath = config['FileDir'][name]
    fs.unlink(`${picpath}/${data}`, (err) => {
      if (err) console.log(err)
      console.log(`删除${data}成功`)
    })
    let keyValue = req.data.keyValue
    let sql = `UPDATE dat_${name} SET pic='' WHERE ${name}_id=${keyValue}`
    let rows = null
    try {
    ...
    this.meta.broadcastMetaData(socket, config.STANDBY, metaDefinition[name], 'delete')
  }
}
